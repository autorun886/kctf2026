        ...
        switch (state) {
            ...
            case 0x9b05688cu: {
                ...
                acc[(round + 5) & 7] ^= honey_fake_lane_round(
                    ...
                    round + 17u);
                round++;
                state = (round < 72) ? 0x9b05688cu : 0x1f83d9abu;
                break;
            }
            ...
            case 0x1f83d9abu:
                ...
                }
                {
                    ...
                    for (int i = 0; i < 32; i++) {
                        round_constants[i] ^= patch[(i + 4) % 24] +
                                              honey_rol32((uint32_t)i * 0x45d9f3bu, i) ^
                                              lane_decoy[(i + 11) & 15];
                    }
                    for (int i = 0; i < 256; i++) {
                        uint8_t k = (uint8_t)(patch[(i >> 4) % 24] >> ((i & 3) * 8));
                        sbox_shipped[i] = (uint8_t)(sbox_shipped[i] ^ k ^ honey_probe_byte((unsigned)i));
                    }
                    for (int i = 0; i < 4; i++) {
                        dispatch_table[i] ^= patch[20 + i] ^ honey_target_words[i];
                    }
